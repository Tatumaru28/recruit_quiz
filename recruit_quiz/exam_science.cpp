...
QuestionList CreatePhysicsExam()
{
	...
	{//等加速度直線運動
		...
		questions.push_back({
			...
			"m/sで等加速度直線運動をはじめた。\n" + to_string(t) +
			"秒後の速度m/s単位で求めよ。",to_string(v0 + a * t)
			});
		...
		v0 = -a * t;	//初速
		questions.push_back({
			"秒速" + to_string(v0) + "mで移動していた車場ブレーキをかけたところ、" +
	...
	{	//浮力
		...
		if (v % 10)		//小数点以下第１位が0以外なら、少数部に文字列に加える
		{
			answer += '.';
			answer += '0' + v % 10;
		}
		...
		int x = uniform_int_distribution<>(20, 50)(rd);	//重さ(空中)
		int y = uniform_int_distribution<>(x / 2, x - 1)(rd);	//高さ(水中)
		int z = x - y + 5;	
		...
		int p0 = uniform_int_distribution<>(20, 50)(rd);	//密度
	...
	{	//ばね
		...
		if (k % 10)		//小数点以下第１位が0以外なら、少数部に文字列に加える
		{
			answer += '.';
			answer += '0' + k % 10;
		}
